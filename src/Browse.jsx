...
class Browse extends Component {
      ...
      componentDidMount() {
        for (var i = 0; i < this.state.games.length; i++) {
          search("EbayAndAmazon", this.state.games[i])
            .on('success', (res) => {
              this.setState((prevState, props) => {
                if(prevState) {
                  var info = prevState.gamesToShow.concat(res.data.Items[0])
                }
                return {
                  gamesToShow: info
                };
      ...
      render() {
         let toRender = []
         let games = this.state.gamesToShow.slice(0,50)

         for(var i = 0; i < 10; i++) {
           toRender.push(games.slice(0 + (5 * i), 5 + (5 * i)))
         }

         return(
           <div className="Browse">
              <p className="welcome">Browse</p>
             <SearchBar history={this.props.history}/>
             <Container className="items-container">
              { toRender.map((row, index) =>
              <Row key={index}>
                { row.map(game =>
                  <Col key={game.id} className="game-node">
                    <Card>
                      ...
                      <CardBody>
                        ...
                        <CardSubtitle className="price-tag">Lowest Price: ${Math.min(parseFloat(game.amazon.price), parseFloat(game.ebay.price))}</CardSubtitle>
